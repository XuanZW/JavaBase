...
#### 线程之间的协作
任务协作时，关键问题是这些问题间的握手。为了实现这种握手，我们使用了相同的基础特性：互斥。在这种情况下，互斥能够确保只有一个任务能够响应某个信号，这样就可以根除任何可能的竞争条件。在互斥之上，我们为任务添加了一种途径，可以将自身挂起，直到某些外部条件发生变化，表示是时候让任务向前开动了为止。这种握手问题可以通过`Object`的方法`wait()`和`notify()`来安全的实现。
* others
